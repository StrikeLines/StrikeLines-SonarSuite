...
class PreprocessorDownsamplingTests(unittest.TestCase):
    def test_chunked_polyphase_resampling_matches_bulk_operation(self):
        source = SyntheticSidescanFile()
        expected = signal.resample_poly(
            source.data.astype(np.float32), up=1, down=4, axis=2
        )
        ...
        self.assertEqual(preprocessor.sonar_data_proc.dtype, np.float32)
